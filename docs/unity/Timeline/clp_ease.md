...
The Curve Editor is the same editor that is used to customize the shape of the blend curves when [blending between clips](clp_blend).
...
- The [pre-extrapolate and post-extrapolate settings](clp_gap_extrap) for the Animation clip and for other Animation clips on the same track.
